...
|---|---|
| ~~**15 Daedric/Dwemer summon tomes and 14 matching scrolls are in no chest and no leveled list**~~ — **no longer true, 2026-09-01** | **Reversed.** They were withheld because Enderal has no Dremora, Xivilai, Daedra, Dwemer or Atronachs. They are named for Enderal now — Entropic and Sinistran for the two demon tiers, Elemental for the Atronach, Starling for the Dwemer, Shade for the Weeping Daedra — so all **175 tomes and 144 scrolls** distribute and the NOT-SOLD column of the test matrix is empty. The `WB_C075_SixDemonBag` tome/scroll asymmetry noted here is resolved by the same change. See `01-gen-renames.ps1` and `17-loot-sublists.ps1` |
