...
| [ ] | **Conjure Craftlord** | `player.addspell XX123E5B` | FireAndForget / TargetLocation | 250 | 370 | - | Calls a Dwemer sage to Vyn for 180 seconds. Modify its stats with the <Reconfigure Craftlord> spell. | NOT-SOLD |
...
| [ ] | **Scroll of Atronach Mark** | `player.additem XX03BA26 1` | 30 | - | Marks a target for 10 seconds. The mark detonates when the target is struck by a summoned or reanimated minion, dealing <30> magic damage in a <15> foot area. | NOT-SOLD |
...
| [ ] | **Scroll of Conjure Craftlord** | `player.additem XX03BFB1 1` | 250 | - | Calls a Dwemer sage to Vyn for 180 seconds. Modify its stats with the <Reconfigure Craftlord> spell. | NOT-SOLD |
...
| [ ] | **Scroll of Conjure Kyrkrim** | `player.additem XX088681 1` | 250 | - | Summons the daedric wolf mount Kyrkrim for 180 seconds. When ridden, low level creatures and people flee in terror. | NOT-SOLD |
...
| [ ] | **Scroll of Conjure Weeping Daedra** | `player.additem XX05F25A 1` | 250 | - | Summons a Weeping Daedra for 120 seconds. It steals large amounts of Health with its magical attacks, but turns to inert and brittle stone when an enemy looks at it. | SUMMON-GAP<br>DEAD-PERK<br>NOT-SOLD |
| [ ] | **Scroll of Conjure Xivilai Lord** | `player.additem XX03BA43 1` | 125 | - | Summons a Xivilai Lord for 60 seconds. When summoned, drains all Magicka and has <200>% of this amount. It casts a deadly bolt that costs <150> Magicka. | NOT-SOLD |
| [ ] | **Scroll of Conjure Xivilai Sorcerer** | `player.additem XX03BA35 1` | 60 | - | Summons a Xivilai Sorcerer for 60 seconds. The caster takes <50> points of fire damage. | NOT-SOLD |
...
- [ ] `WB_Con_Curse2_Spell_AtronachMark` -- **Atronach Mark** is not offered by any merchant and does not drop
- [ ] `WB_Con_Human5_Spell_ConjureCraftlord` -- **Conjure Craftlord** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora5_Spell_ConjureDremoraAssassin_NPC` -- **Conjure Dremora Assassin** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora4_Spell_ConjureDremoraChampion_NPC` -- **Conjure Dremora Champion** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora2_Spell_ConjureDremoraChurl_NPC` -- **Conjure Dremora Churl** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora4_Spell_ConjureDremoraHonorGuard_NPC` -- **Conjure Dremora Honor Guard** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora4_Spell_ConjureDremoraMentor` -- **Conjure Dremora Mentor** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora3_Spell_ConjureDremoraPitFighter_NPC` -- **Conjure Dremora Pit Fighter** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora4_Spell_ConjureHerne_NPC` -- **Conjure Herne** is not offered by any merchant and does not drop
...
- [ ] `WB_Con_Dremora5_Spell_ConjureWeepingDaedra_NPC` -- **Conjure Weeping Daedra** is not offered by any merchant and does not drop
- [ ] `WB_Con_Xivilai4_Spell_ConjureXivilaiLord_NPC` -- **Conjure Xivilai Lord** is not offered by any merchant and does not drop
- [ ] `WB_Con_Xivilai3_Spell_ConjureXivilaiSorcerer_NPC` -- **Conjure Xivilai Sorcerer** is not offered by any merchant and does not drop
- [ ] `WB_Con_Dremora4_Spell_SixDemonBag` -- **Six Demon Bag** is not offered by any merchant and does not drop

**Note the inconsistency:** 15 tomes are withheld but only 14 of the matching scrolls are. 
