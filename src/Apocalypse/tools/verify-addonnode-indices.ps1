...
#
# -Upstream checks Enai's ORIGINAL tree (reference/mods/Apocalypse/esp) instead of ours. That is
# how the WB_IllusionNightmare_MPS_Seidsigil 110-vs-_00E_MPSWildWaveFlames collision was found in
# the first place, and it still reports it - the fix (110 -> 746) lives in OUR tree, which is what
# this checks by default. Verifying the upstream tree tells you nothing about what ships.
[CmdletBinding()]
...
$apocTree = if ($Upstream) { Join-Path $repo 'reference\mods\Apocalypse\esp' }
            else           { Join-Path $repo 'src\Apocalypse\ApocalypseESP' }
"checking: $apocTree"
$all += Get-Nodes $apocTree 'Apocalypse'
