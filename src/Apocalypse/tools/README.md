...
| 7 | `07-place-vendor-tomes.ps1` | Writes all 163 distributed tomes **directly** to six named merchants, tiered by the gold in their chest — into SureAI's own `<Merchant>_CustomMerchandise` hooks, so **no container record of any master is overridden**. This is what actually makes the spells obtainable; the vendor leveled lists no longer carry them. Migrates away any leftover chest override. Idempotent — always rebuilds from the Forgotten Stories record |
...
| 17 | `17-loot-sublists.ps1` | Rebuilds the five `ZP_Apoc_Tomes_R*` sublists and `ZP_Apoc_Scrolls` from our own tree — the **163** tomes and **133** scrolls that `00-cut-summons.ps1` does not withhold. Step 2 does the same job at *staging* FormKeys, which step 5 remaps — so 2 cannot be re-run against the built tree, and this is its post-merge twin. On a full regeneration it rewrites them byte-identically. Idempotent |
...
- **The 15 Daedra/Dwemer summons are all renamed, and 3 of them ship.** They used to be withheld from every vendor and loot list because Enderal has none of those things. `01-gen-renames.ps1` now maps Dremora → **Entropic**, Xivilai → **Sinistran**, the Weeping Daedra → **Shade**, Atronach → **Elemental** (Enderal ships four Elementals of its own) and Dwemer → **Starling** (Enderal's `Dwarven*Race`s *are* the Starling constructs), so naming withholds nothing any more. What still withholds twelve of them is **testing**: they had never been distributed, so nobody had cast one, and when three were finally examined two were broken (Herne had no arrows, the Craftlord was naked). Herne, the Entropic Assassin and the Craftlord are fixed and ship; the rest wait. **`00-cut-summons.ps1` is the single definition** — steps 2, 7 and 17 dot-source it, and each asserts an exact total, so moving a spell out of the list without updating the counts fails loudly. Note the meshes are still Bethesda's Dremora and Xivilai; the rename fixes the names, not the models.
- **Never use a bare race or school word as a rename key.** The table does plain substring replacement over the whole record, so `'Conjuration'` would rewrite `WB_Conjuration_ConjureDremoraAssassin_Global_Health` — an EditorID a description names in a live `<Global=…>` lookup, which would then resolve to nothing. Anchor to the field instead (`'    Value: Alteration'`, verified to occur exactly once in the tree). And put the article keys (`'a Dremora Champion'` → `'an Entropic Champion'`) *before* the bare ones, or you ship "Summons a Entropic Champion".
