...
| 7 | `07-place-vendor-tomes.ps1` | Writes all 175 tomes **directly** to six named merchants, tiered by the gold in their chest — into SureAI's own `<Merchant>_CustomMerchandise` hooks, so **no container record of any master is overridden**. This is what actually makes the spells obtainable; the vendor leveled lists no longer carry them. Migrates away any leftover chest override. Idempotent — always rebuilds from the Forgotten Stories record |
...
| 17 | `17-loot-sublists.ps1` | Rebuilds the five `ZP_Apoc_Tomes_R*` sublists and `ZP_Apoc_Scrolls` from our own tree, so all **175** tomes and **144** scrolls are in the loot tables. Step 2 does the same job at *staging* FormKeys, which step 5 remaps — so 2 cannot be re-run against the built tree, and this is its post-merge twin. On a full regeneration it rewrites them byte-identically. Idempotent |
...
- **The 15 Daedra/Dwemer summons are no longer cut — they are renamed.** They used to be withheld from every vendor and loot list because Enderal has none of those things, which cost the player a sixth of the spellbook to avoid a naming problem. `01-gen-renames.ps1` now maps Dremora → **Entropic**, Xivilai → **Sinistran**, the Weeping Daedra → **Shade**, Atronach → **Elemental** (Enderal ships four Elementals of its own) and Dwemer → **Starling** (Enderal's `Dwarven*Race`s *are* the Starling constructs). `$removed` in steps 2 and 7 is now empty; keep the variable, it is the documented lever if a summon ever has to be withheld again. Note the meshes are still Bethesda's Dremora and Xivilai — the rename fixes the names, not the models.
