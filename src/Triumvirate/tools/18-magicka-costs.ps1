...
# Put Triumvirate's 75 spells on Enderal's mana scale, and stop the engine recomputing them.
#
...
# ManualCostCalc, so every SureAI cost is a number a designer typed. Triumvirate does the opposite:
# NONE of its 75 tome-taught spells sets the flag, so all 75 costs are whatever the Creation Kit's
# formula produced, and the duration term dominates - which is why the archetypes' long-duration
# summons and auras are the worst offenders.
#
...
#
# Triumvirate's medians were 50 / 66 / 168 / 323 / 1189 with a 1484 ceiling. The master tier is
# 15x Enderal's, and Enderal's mana pool makes that fatal rather than merely expensive: the player
# gains +8 max mana per level and only when they spend that level's attribute choice on it
# (_00e_epupdatefunctions.psc), so even a mage who never picks anything else ends a playthrough
# near 400-500. Every one of the five archetypes' capstones was uncastable.
#
...
# - Call the Glacier Hound is a 65-cost master summon in Enai's tree - does not fall to single
# digits. Resulting medians 45 / 55 / 75 / 125 / 180 against Enderal's 21 / 40 / 55 / 65 / 80, and a
# 225 ceiling well under Enderal's own 310.
#
# SCOPE. Only the 75 spells the mod's tomes teach. Triumvirate has no player-facing variants
# outside that set (the generator checks, and finds none); everything else in Spells/ is a proc,
# hazard, subspell or NPC copy that a script fires, where no magicka is ever billed.
#
...
$orig = Join-Path $repo 'reference\mods\Triumvirate\esp'
$mine = Join-Path $repo 'src\Triumvirate\TriumvirateESP'
...
$ratio = @{ Novice = 0.90; Apprentice = 0.80; Adept = 0.45; Expert = 0.38; Master = 0.15 }
# The 25th percentile of Enderal's own tome-taught spells at each tier. A ported spell never ends
# up cheaper than a quarter of what Enderal already sells at its tier - which matters here because
# Triumvirate's Expert tier is bimodal (five spells at 30-62, ten at 278-453) and a flat ratio would
# push the cheap half, already perfectly Enderal-shaped, down into the teens.
...
if ($taught.Count -ne 75) { throw "expected 75 tome-taught spells, found $($taught.Count)" }
...
#
# Triumvirate currently has none. The scan stays in because a regeneration against a newer upstream
# could introduce one, and a silently unscaled player spell is exactly the failure this fix is for.
$lockstep = @()
